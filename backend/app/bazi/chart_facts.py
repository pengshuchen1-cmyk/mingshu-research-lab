...
class ChartFacts:
    ...
    def from_dict(cls, value: Mapping[str, object]) -> ChartFacts:
        ...
        if not isinstance(hidden_raw, Mapping):
            raise ValueError(  # noqa: TRY004 - established domain contract
                "invalid attached ChartFacts hidden stems"
            )
        hidden = tuple(
        ...
        for name, item in (
            ...
            if not isinstance(item, Mapping):
                raise ValueError(  # noqa: TRY004 - established domain contract
                    f"invalid attached ChartFacts {name}"
                )
        elements = value.get("element_counts") or {}
        if not isinstance(elements, Mapping):
            raise ValueError(  # noqa: TRY004 - established domain contract
                "invalid attached ChartFacts elements"
            )
        return cls(
...
def chart_facts_from_chart(chart: Mapping[str, object]) -> ChartFacts:
    ...
    if not isinstance(raw, Mapping):
        raise ValueError(  # noqa: TRY004 - established domain contract
            "chart is missing canonical ChartFacts; rebuild it with the rule engine"
        )
