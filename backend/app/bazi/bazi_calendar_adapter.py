...
def _solar_datetime(solar: object) -> datetime:
    # Domain values are wall-clock China-standard civil times, not instants.
...
def normalize_birth_input(value: BirthInput) -> CalendarEvidence:
    ...
    if value.hour is not None and value.minute is not None:
        civil = datetime(  # noqa: DTZ001 - intentional civil wall time
            converted.year,
            converted.month,
            converted.day,
            value.hour,
...
def jie_boundaries(year: int) -> tuple[JieBoundary, ...]:
    ...
    if any(left.at >= right.at for left, right in pairwise(boundaries)):
