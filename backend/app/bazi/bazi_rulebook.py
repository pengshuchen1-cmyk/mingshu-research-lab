...
RULE_DIR = Path(__file__).resolve().parent / "rules" / "bazi_skill"
...
def _known_citations(manifest: dict[str, object]) -> set[str]:
    ...
    if missing:
        raise RuleBookError(f"unknown citation in manifest: {min(missing)}")
    return set(declared)
...
def load_rulebook(rule_dir: Path | None = None) -> RuleBook:
    ...
    for entry in file_entries:
        ...
        for item in payload["rules"]:
            ...
            if unknown:
                raise RuleBookError(f"unknown citation: {min(unknown)}")
    ...
    if missing_sections:
        raise RuleBookError(f"missing section: {min(missing_sections)}")
