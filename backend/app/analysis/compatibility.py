...
from ..bazi.bazi_constants import CONTROLLING, GENERATING, NAYIN_ELEMENT, STEM_ELEMENTS
    ...
    for d in dimensions:
        ...
        if pct >= 0.7:
            ...
            if "六合" in d.get("text", ""):
                ...
                reasons.append("五行互补性良好，能在生活和性格上互补。")
            elif "喜用神互补" in label and pct >= 0.6:
                reasons.append("喜用神互补，对方的优势能够补充自己的短板。")
            elif "天干五合" in label:
                reasons.append("天干五合，沟通默契度较高，容易理解对方的意图。")
            elif "纳音" in label and pct >= 0.7:
                reasons.append("纳音配对相生，气场和谐。")
    if not reasons:
