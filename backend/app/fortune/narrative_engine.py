...
from .chart_fingerprint import build_chart_fingerprint
...
def _elements(luck_item: dict) -> list[str]:
    ...
    return [
        item
        for item in [luck_item.get("gan_element", ""), luck_item.get("zhi_element", "")]
        if item
    ]
...
def _yearly_fingerprint_notes(chart: dict, relation: str) -> dict[str, str]:
    ...
    if fp["has_strong_wealth"] and fp["has_strong_officer_killing"]:
        career = (
            "对这类命盘来说，本年更要把客户收益、岗位责任、项目交付和规则边界放在同一张清单里管理。"
        )
        ...
        career = (
            "对这类命盘来说，本年适合靠作品、表达、技术输出和个人标签推进，但合作分工要写清楚。"
        )
    ...
    advice = (
        f"年度行动不要只看流年干支，还要回到命盘标签：{'、'.join(fp['chart_summary_tags'][:5])}。"
    )
    return {
        "base": base,
        "career": career,
...
def _monthly_fingerprint_notes(
    chart: dict, relation: str, month_name: str, pillar: str
    ...
    pillar_text = "".join(
        pillars.get(key, {}).get("pillar", "") for key in ["year", "month", "day", "hour"]
    )
    counts_text = (
    ...
    if fp["spouse_palace_element"] in fp["unfavorable_elements"]:
        ...
        relationship = (
            f"关系上夫妻宫{fp['day_branch']}关联喜用，{month_name}适合用具体安排增加信任。"
        )
    else:
        relationship = (
            f"关系上夫妻宫{fp['day_branch']}需结合真实互动观察，{month_name}不要只凭情绪下判断。"
        )
...
def build_luck_stage_narrative(chart: dict, luck_item: dict) -> str:
    ...
    """
    ten_god = luck_item.get("ten_god") or get_ten_god(
        chart.get("day_master", ""), luck_item.get("gan", "")
    )
...
def build_yearly_narrative(chart: dict, yearly_item: dict) -> dict:
    ...
    element_text = "、".join(elements) or "五行信息暂不完整"
    branch_ten_god = yearly_item.get("branch_ten_god") or _main_branch_ten_god(
        chart, yearly_item.get("zhi", "")
    )
    ...
    keywords = _unique(
        list(info["keywords"])
        + [branch_ten_god, relation]
        + [item.get("label", "") for item in branch_relations],
        8,
    )
    ...
    return {
        ...
        "suitable_actions": _unique(
            list(suitable)
            + list(MONTHLY_TEN_GOD_EVENTS.get(ten_god, MONTHLY_TEN_GOD_EVENTS["未知"])["suitable"]),
            6,
        ),
        "actions_to_avoid": _unique(
            list(avoid)
            + list(MONTHLY_TEN_GOD_EVENTS.get(ten_god, MONTHLY_TEN_GOD_EVENTS["未知"])["avoid"]),
            6,
        ),
    }
...
def build_monthly_narrative(chart: dict, monthly_item: dict) -> dict:
    ...
    suitable, avoid = _actions_for_relation(relation)
    suitable_actions = _unique(
        list(event_info["suitable"]) + suitable + monthly_item.get("rule_advices", [])[:1], 6
    )
