...
from ..bazi.chart_facts import chart_facts_from_chart
...
def _day_master_profile(chart: dict) -> str:
    ...
    combo_text = {
        (
            "火",
            "从弱",
        ): "火日主从弱时，不宜只靠热情硬冲；现实中更像小灯放在风雨里，要借规则、资源、职位平台和现实条件保护火苗。",
        (
            "火",
            "身弱",
        ): "火日主身弱时，表达力需要被环境点燃，适合先积累作品、贵人和稳定场景，再逐步增加曝光。",
        (
            "火",
            "中和",
        ): "火日主中和时，既能表达也能收住，适合在行动和复盘之间来回校准，避免只凭一阵热度做决定。",
        (
            "火",
            "身强",
        ): "火日主身强时，行动、表达和领导欲更明显，适合主动争取舞台，但要留意急躁、面子和睡眠消耗。",
        (
            "水",
            "从弱",
        ): "水日主从弱时，流动性被现实土火牵引，适合顺着责任、财务、项目和平台机会走，不宜长期停在犹豫里。",
        (
            "水",
            "身弱",
        ): "水日主身弱时，信息和精力都要先蓄起来，适合减少无效奔波，把沟通、学习和休息节奏排稳。",
        (
            "水",
            "中和",
        ): "水日主中和时，信息处理和资源调度较灵活，关键是把想法落到具体计划，不要只流动不沉淀。",
        (
            "水",
            "身强",
        ): "水日主身强时，思路多、变化快、人脉流动强，适合做沟通资源型事务，但要防止方向过散。",
        (
            "土",
            "身强",
        ): "土日主身强时，承载力和稳定性明显，适合管资源、管流程、做长期积累，但要避免固守旧经验。",
        ("土", "身弱"): "土日主身弱时，现实责任容易压身，适合先搭框架、找帮手、分阶段完成任务。",
        (
            "木",
            "身强",
        ): "木日主身强时，成长欲和主见较明显，适合规划、教育、创意与开拓，但要修剪分支。",
        (
            "金",
            "身强",
        ): "金日主身强时，规则感、执行力和取舍能力明显，适合技术、管理、金融或标准化事务。",
...
def _top_ten_god_story(chart: dict) -> str:
    ...
    top_items = [
        (name, int(value))
        for name, value in sorted(counts.items(), key=lambda item: (-int(item[1]), item[0]))
        if int(value) > 0
    ][:3]
    meaning = {
    ...
    parts = [
        f"{name}{count}个：{meaning.get(name, '需要结合位置继续判断')}" for name, count in top_items
    ]
...
def _position_ten_god_story(chart: dict) -> str:
    ...
    for key in ("year", "month", "day", "hour"):
        ...
        hidden = [
            item.get("ten_god", "") for item in hidden_stems.get(key, []) if item.get("ten_god")
        ]
...
def _build_chart_signature_text_impl(chart: dict, prefix: str = "本盘差异依据") -> str:
    ...
    if isinstance(facts, dict):
        ...
        for label, key, pillar in zip(pillar_names, pillar_keys, pillars):
            ...
            structure.append(f"{label}{pillar}：透干{item.get('gan', '未知')}，藏干{hidden}")
