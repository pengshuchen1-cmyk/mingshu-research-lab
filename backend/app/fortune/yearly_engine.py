...
from ..bazi import four_pillars_engine
...
from .narrative_engine import build_yearly_narrative
from .report_diversity import build_chart_signature_text
...
def _relation_score(
    elements: list[str], favorable: set[str], unfavorable: set[str]
) -> tuple[int, str]:
    """判断年度五行与喜忌的关系。"""
    score = 0
    has_favorable = False
        ...
        if start_year <= target_year <= end_year:
            ...
            try:
                ...
                if boundary.year == target_year and (boundary.month, boundary.day) != (1, 1):
                    boundary_note = (
                        f" 该年于{boundary.month}月{boundary.day}日换入此运，年初仍属上一运。"
                    )
            except ValueError:
                pass
            return (
...
def _month_attention(
    ...
    if monthly_data is None:
        try:
            from .monthly_engine import analyze_monthly_fortune
...
def _enhance_wealth_text(
    base_text: str,
    good_months: list[str],
    bad_months: list[str],
    ten_god: str = "",
    relation: str = "",
    overall_level: str = "",
...
def _enhance_relationship_text(
    base_text: str,
    good_months: list[str],
    bad_months: list[str],
    ...
    *,
    ...
    if include_monthly_analysis:
        try:
            ...
            year_zhi = (
                get_year_pillar(target_year)[1] if len(get_year_pillar(target_year)) >= 2 else ""
            )
            ...
            # 增强文本
            enhanced_career = _enhance_career_text(
                narrative["career_text"], career_good, career_bad
            )
            enhanced_wealth = _enhance_wealth_text(
                narrative["wealth_text"], wealth_good, wealth_bad, ten_god, relation, overall_level
            )
            enhanced_relationship = _enhance_relationship_text(
                narrative["relationship_text"], rel_good, rel_bad, peach_months, gender
            )
