...
def build_month_context(
    ...
    ctx["month_ten_god_group"] = (
        "wealth"
        if ctx["is_wealth_month"]
        else "officer"
        if ctx["is_officer_month"]
        else "output"
        if ctx["is_output_month"]
        else "resource"
        if ctx["is_resource_month"]
        else "peer"
        if ctx["is_peer_month"]
        else ""
    )
    ...
    ctx["clash_any"] = any(
        [
            ctx["clash_year_branch"],
            ctx["clash_month_branch"],
            ctx["clash_day_branch"],
            ctx["clash_hour_branch"],
        ]
    )
    ...
    ctx["activate_spouse_palace"] = ctx["clash_day_branch"] or ctx["month_zhi"] in (
        "子",
        "午",
        "卯",
        "酉",
...
def match_condition(cond: dict, ctx: dict) -> dict:
    ...
    if ctype == "ten_god":
        ...
        matched = any(
            e in ctx.get("month_element", "") or e in ctx.get("month_zhi_element", "")
            for e in value
        )
...
def _select_variant(
    event_type: str, evidence_list: list, variants_db: dict, month_index: int = 0
) -> dict | None:
    ...
    seed_str = (
        ...
        + "".join(str(e.get("type", "")) for e in evidence_list[:3])
    )
...
def _evidence_dimensions(evidence_list: list[dict]) -> set[str]:
    ...
    for item in evidence_list:
        etype = item.get("type", "")
        if (
            etype in ("ten_god", "ten_god_group")
            or etype.startswith("is_")
            or etype.startswith("activate_")
        ):
            dimensions.add("十神")
        if etype in (
            "favorable_relation",
...
def _evaluate_source_confidence(
    cand: dict, onto: dict, source_registry: dict | None = None
) -> dict:
...
def _evaluate_event_confidence(
    ...
    if (
        level == "high"
        and onto.get("category")
        and not source_confidence["source_has_category_match"]
    ):
...
def merge_related_event_clusters(events: list[dict]) -> list[dict]:
    ...
    for spec in clusters:
        ...
        if len(cluster) < 2:
            continue
        best = sorted(
            cluster, key=lambda ev: (-float(ev.get("score", 0) or 0), ev.get("event_type", ""))
        )[0]
...
def _enrich_candidate(
    cand: dict, ontology: dict, variant: dict | None, source_registry: dict, ctx: dict | None = None
) -> dict:
    ...
    subtypes = (
        _infer_nobleman_subtypes(ctx, cand.get("evidence", []))
        if et
        in {
            "nobleman_help",
            "mentor_advice",
    ...
    for c in bridge_candidates:
        if c.get("trigger_count", 0) >= 2:
            v = _select_variant(
                ...
                c.get("evidence", []),
                vdb,
                ctx.get("month_index", 0) if ctx else 0,
            )
...
def infer_monthly_likely_events_full(
    ...
    from .monthly_event_inference_engine import infer_monthly_likely_events_enhanced
    ...
    bridge_candidates = sorted(
        best_by_type.values(), key=lambda item: -float(item.get("score", 0) or 0)
    )
    ...
    for b in bridge_candidates:
        if b.get("trigger_count", 0) >= 2 and b.get("event_type") not in base_types:
            v = _select_variant(
                b["event_type"], b.get("evidence", []), vdb, ctx.get("month_index", 0) if ctx else 0
            )
...
def build_year_monthly_event_results(
    chart: dict, monthly_data: list, yearly_data=None, luck_data=None
) -> list:
    from .monthly_event_inference_engine import postprocess_monthly_events
