...
from ..bazi.bazi_constants import BRANCH_HIDDEN_STEMS, BRANCH_MAIN_ELEMENTS, STEM_ELEMENTS
from ..bazi.ten_gods import get_hidden_stem_ten_gods
...
def _sorted_keys_by_value(data: dict, reverse: bool = True) -> list[str]:
    ...
    return [
        ...
        for key, value in sorted(
            data.items(), key=lambda item: (-item[1], item[0]) if reverse else (item[1], item[0])
        )
        if value > 0
    ]
...
def _canonical_inputs(
    chart: dict,
) -> tuple[dict, dict, dict, str, list[str], list[str], str, list[str]]:
...
def build_chart_fingerprint(chart: dict) -> dict:
    ...
    weak_ten_gods = [
        name
        for name in ["比肩", "劫财", "食神", "伤官", "偏财", "正财", "七杀", "正官", "偏印", "正印"]
