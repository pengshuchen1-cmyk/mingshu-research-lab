...
from .narrative_engine import build_monthly_narrative
from .report_diversity import build_chart_signature_text
from .yearly_engine import TEN_GOD_THEMES
...
RULES_PATH = Path(__file__).resolve().parent / "rules" / "monthly_event_rules.json"
SOURCE_REGISTRY_PATH = Path(__file__).resolve().parent / "rules" / "source_registry.json"
...
def analyze_monthly_fortune(chart: dict, target_year: int) -> list[dict]:
    ...
    for month in range(1, 13):
        ...
        rule_tags, rule_events, rule_advices, rule_source_ids, rule_basis = _match_event_rules(
            ten_god, elements, branch_relations, rules
        )
