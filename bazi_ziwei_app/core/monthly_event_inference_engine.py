...
REALITY_EVENT_RULES = {
    "vehicle_safety": {
        "min_trigger_count": 2,
        "trigger_conditions": ["七杀或压力星被引动", "年支或时支被冲动", "火金工具象明显", "忌神参与", "流月主事件池命中"],
    },
    "property_housing": {
        "min_trigger_count": 2,
        "trigger_conditions": ["土象被引动", "印星或财星参与", "日支居住位被冲", "喜用五行助力", "流月主事件池命中"],
    },
    "shop_property": {
        "min_trigger_count": 2,
        "trigger_conditions": ["财星带动经营", "土象场地事务", "印星承载资产", "项目或客户信号出现", "流月主事件池命中"],
    },
    "contract_document": {
        "min_trigger_count": 2,
        "trigger_conditions": ["官杀规则星被引动", "伤官表达与规则相遇", "月支工作位被冲", "金象文书规则明显", "流月主事件池命中"],
    },
    "illness_symbol_attention": {
        "min_trigger_count": 2,
        "trigger_conditions": ["忌神压身", "官杀压力较重", "冲动身体状态", "五行偏枯或火土金压力", "流月主事件池命中"],
    },
    "social_drinking": {
        "min_trigger_count": 2,
        "trigger_conditions": ["比劫朋友星被引动", "桃花地支带来应酬", "忌神带来人情消耗", "日支被冲易口舌", "流月主事件池命中"],
    },
    "favor_obligation": {
        "min_trigger_count": 2,
        "trigger_conditions": ["比劫或劫财明显", "朋友同辈求助", "财星被消耗", "关系位被冲", "流月主事件池命中"],
    },
    "official_dispute": {
        "min_trigger_count": 2,
        "trigger_conditions": ["官杀规则压力", "伤官见官", "七杀遇冲", "金象规则或罚单信号", "流月主事件池命中"],
    },
    "debt_loss": {
        "min_trigger_count": 2,
        "trigger_conditions": ["比劫制财", "财星为忌", "冲动带来计划外支出", "偏财过重", "流月主事件池命中"],
    },
    "client_payment": {
        "min_trigger_count": 2,
        "trigger_conditions": ["正财或偏财被引动", "食伤生财", "喜用五行助力", "年度喜用相关", "流月主事件池命中"],
    },
    "relationship_progress": {
        "min_trigger_count": 2,
        "trigger_conditions": ["财官关系星被引动", "子午卯酉桃花地支", "日支关系位被触发", "喜用五行助力", "流月主事件池命中"],
    },
    "family_issue": {
        "min_trigger_count": 2,
        "trigger_conditions": ["年支家庭位被冲", "印星或土象家庭承载", "房产田宅信号出现", "长辈或旧事被引动", "流月主事件池命中"],
    },
}
...
def infer_monthly_likely_events(
    ...
    has_clash = monthly_item.get("has_clash", any(r.get("relation_type") == "六冲" for r in branch_rels) if branch_rels else False)
    ...
    conditions = [
        ...
        ten_god_counts.get("偏财", 0) > ten_god_counts.get("正财", 0) + 1 if ten_god_counts else False,
    ...
    conditions = [
        ...
        has_clash and _is_unfavorable(gan_element, unfavorable) and _is_unfavorable(zhi_element, unfavorable),
        ten_god in ("偏财", "七杀"),
    ...
    for event_type, score in sorted_events:
        if score >= 22 and len(top_events) < 5:
            ...
            probability_level = _PROB_HIGH if score >= 60 else _PROB_MED if score >= 40 else _PROB_LOW
            top_events.append({
                ...
                "score": round(score, 1),
                "reason": _generate_reason(event_type, ten_god, gan_element, zhi_element, has_clash, zhi),
    ...
    event_opportunities = [e for e in top_events if e["score"] >= 50 and "风险" not in e.get("advice", "")]
...
def _generate_reason(event_type: str, ten_god: str, gan_element: str, zhi_element: str, has_clash: bool, zhi: str = "") -> str:
    """生成事件触发原因（含具体干支五行）。"""
    ...
    base_reason = reasons.get(event_type, f"流月{ten_god}和{gan_element}{zhi_element}五行被引动{clash_note}，相关事务容易被触发。")
    return base_reason
...
def _generate_advice(event_type: str) -> str:
    ...
    return advices.get(event_type, "建议结合现实情况灵活应对，重要决策留出核实和确认时间，不因命理提示而冲动行事。")
...
def _identify_clash_influence_area(zhi: str, chart: dict) -> dict:
    ...
    clashes = []
    if zhi == year_zhi:
        clashes.append({"_type": "年支", "_label": "家庭/长辈/外部环境", "_events": ["family_issue", "old_contact", "travel_traffic", "public_expression"]})
    if zhi == month_zhi:
        clashes.append({"_type": "月支", "_label": "工作/职场/流程/上级", "_events": ["contract_document", "boss_pressure", "career_change", "project_progress"]})
    if zhi == day_zhi:
        clashes.append({"_type": "日支", "_label": "伴侣/关系/居住/自我", "_events": ["relationship_conflict", "relationship_progress", "property_housing", "sleep_issue"]})
    if zhi == hour_zhi:
        clashes.append({"_type": "时支", "_label": "出行/长期规划/子女/副业", "_events": ["travel_traffic", "career_change", "old_contact", "study_exam"]})
    return {"clashes": clashes, "has_clash": len(clashes) > 0}


def _get_clash_driven_events(zhi: str, chart: dict, ten_god: str, gan_element: str, zhi_element: str, favorable: list, unfavorable: list) -> list[dict]:
    """根据地支冲击目标生成差异化事件及加分。"""
    ...
    for c in info.get("clashes", []):
        for etype in c["_events"]:
            ...
            base_score = 30
            if _is_unfavorable(gan_element, unfavorable) or _is_unfavorable(zhi_element, unfavorable):
                base_score += 15
            ...
            events.append({
                ...
                "score": base_score,
                "reason": reason_map.get(etype, f"流月{zhi}地支{c['_type']}被冲{c['_label']}，相关事务容易被触发"),
...
def _get_season_events(month_num: int, gan_element: str, zhi_element: str, favorable: list) -> list[dict]:
    """基于月份季节生成季节事件加分。"""
    season_map = {
        "spring": {"months": [1, 2, 3], "elements": ["木"], "label": "春季·生发", "events": [("study_exam", 15), ("project_progress", 15), ("public_expression", 10)]},
        "summer": {"months": [4, 5, 6], "elements": ["火"], "label": "夏季·旺盛", "events": [("public_expression", 15), ("travel_traffic", 10), ("relationship_progress", 10)]},
        "autumn": {"months": [7, 8, 9], "elements": ["金"], "label": "秋季·收敛", "events": [("contract_document", 15), ("project_progress", 15), ("legal_compliance", 10)]},
        "winter": {"months": [10, 11, 12], "elements": ["水"], "label": "冬季·收藏", "events": [("sleep_issue", 15), ("emotional_pressure", 10), ("kidney_fatigue", 10)]},
    }
    events = []
        ...
        if month_num in info["months"]:
            for etype, bonus in info["events"]:
                if _is_favorable(gan_element, favorable):
                    events.append({"event_type": etype, "bonus": bonus, "season_label": info["label"]})
    return events


def _check_major_event_conditions(event_type: str, matched_count: int, total_conditions: int) -> bool:
    """检查大事件是否满足最少触发条件数。"""
    min_required = {"property_housing": 2, "travel_traffic": 2, "wealth_inflow": 2, "wealth_outflow": 2}
...
def postprocess_monthly_events(all_results: list[dict]) -> list[dict]:
    ...
    if overused:
        for r in all_results:
            ...
            if len(r["top_events"]) < 2:
                ...
                remaining = sorted([(k, v) for k, v in score_map.items() if k not in overused], key=lambda x: -x[1])
                for et, sc in remaining[:2]:
                    evt_info = EVENT_TYPES.get(et, {"label": et, "category": "其他"})
                    r["top_events"].append({
                        "event_type": et, "label": evt_info["label"], "category": evt_info["category"],
                        "probability_level": "需观察", "score": sc,
                        "reason": "作为月度事件补充参考。",
    ...
    for i in range(1, len(all_results)):
        ...
        if prev_types and curr_types and prev_types == curr_types:
            ...
            alternatives = [(k, v) for k, v in sorted(score_map.items(), key=lambda x: -x[1]) if k not in prev_types and v >= 20]
            if alternatives:
...
def build_year_monthly_event_results(
    ...
    results = [
        infer_monthly_likely_events(chart, item, yearly_data, luck_data)
...
def infer_monthly_likely_events_enhanced(
    chart: dict, monthly_item: dict, yearly_data: dict | None = None, luck_data: dict | None = None,
    ...
    for evt_type in list(EVENT_TYPES.keys()):
        ...
        if evt_type == "wealth_inflow":
            total = 6
            if is_cai: matched += 1; factors.append("财星被引动")
            if _is_favorable(gan_element, favorable) or _is_favorable(zhi_element, favorable): matched += 1; factors.append("流月五行为喜用")
            if is_shishen: matched += 1; factors.append("食伤生财")
            if yearly_data and yearly_data.get("relation_to_favorable") == "喜用相关": matched += 1; factors.append("年度喜用相关")
            if is_cai and not has_clash: matched += 1; factors.append("财星平稳无冲")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 10

        # ---- 支出破财 ----
        elif evt_type == "wealth_outflow":
            total = 6
            if is_cai and _is_unfavorable(gan_element, unfavorable): matched += 1; factors.append("财星为忌")
            if is_bi_jie: matched += 1; factors.append("比劫制财")
            if has_clash and _is_unfavorable(zhi_element, unfavorable): matched += 1; factors.append("冲+忌神")
            if ten_god_counts.get("偏财", 0) > ten_god_counts.get("正财", 0) + 1: matched += 1; factors.append("偏财偏重")
            if is_bi_jie and has_clash: matched += 1; factors.append("比劫+冲")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 10
        ...
        # ---- 客户回款/订单收益 ----
            ...
            if is_cai: matched += 1; factors.append("财星被引动")
            if is_shishen: matched += 1; factors.append("食伤生财")
            if _is_favorable(gan_element, favorable) or _is_favorable(zhi_element, favorable): matched += 1; factors.append("喜用助力")
            if yearly_data and yearly_data.get("relation_to_favorable") == "喜用相关": matched += 1; factors.append("年度喜用相关")
            if not has_clash and is_cai: matched += 1; factors.append("财星平稳利结算")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 11
        ...
        # ---- 破财漏财/借支担保 ----
            ...
            if is_bi_jie: matched += 1; factors.append("比劫制财")
            if is_cai and _is_unfavorable(gan_element, unfavorable): matched += 1; factors.append("财星为忌")
            if has_clash: matched += 1; factors.append("冲动带来计划外支出")
            if ten_god_counts.get("偏财", 0) > ten_god_counts.get("正财", 0): matched += 1; factors.append("偏财机会伴随波动")
            if _is_unfavorable(gan_element, unfavorable) or _is_unfavorable(zhi_element, unfavorable): matched += 1; factors.append("忌神参与")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 11
        ...
        # ---- 房产居住 ----
            ...
            is_tu = zhi_element == "土" or gan_element == "土"
            has_day_clash = has_clash and zhi == chart.get("pillars", {}).get("day", {}).get("zhi", "")
            if is_tu: matched += 1; factors.append("流月土旺")
            if is_yin and _is_favorable(gan_element, favorable): matched += 1; factors.append(f"印星+喜用")
            if is_cai and _is_favorable(zhi_element, favorable): matched += 1; factors.append(f"财星+喜用")
            if has_day_clash: matched += 1; factors.append("冲日支居住")
            if is_tu and has_clash: matched += 1; factors.append("土+冲")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 10
        ...
        # ---- 房车店铺/大件添置 ----
            ...
            total = 6
            is_tu = zhi_element == "土" or gan_element == "土"
            if is_cai: matched += 1; factors.append("财星动资产")
            if is_tu: matched += 1; factors.append("土象房产店铺")
            if is_yin: matched += 1; factors.append("印星主承载")
            if has_clash and zhi == chart.get("pillars", {}).get("day", {}).get("zhi", ""): matched += 1; factors.append("冲日支居住")
            if _is_favorable(gan_element, favorable) or _is_favorable(zhi_element, favorable): matched += 1; factors.append("喜用助力")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 11
        ...
        # ---- 店铺门面/经营场地 ----
            ...
            total = 6
            is_tu = zhi_element == "土" or gan_element == "土"
            if is_cai: matched += 1; factors.append("财星带动经营")
            if is_tu: matched += 1; factors.append("土象场地事务")
            if is_yin: matched += 1; factors.append("印星主固定资源")
            if ten_god in ("正财", "偏财", "正印", "偏印"): matched += 1; factors.append(f"{ten_god}关联经营承载")
            if _is_favorable(gan_element, favorable) or _is_favorable(zhi_element, favorable): matched += 1; factors.append("喜用助力")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 11
        ...
        # ---- 出行交通 ----
            ...
            total = 5
            has_year_hour_clash = has_clash and (zhi == chart.get("pillars", {}).get("year", {}).get("zhi", "") or zhi == chart.get("pillars", {}).get("hour", {}).get("zhi", ""))
            if has_year_hour_clash: matched += 1; factors.append("冲年/时支出行")
            if has_clash and _is_unfavorable(gan_element, unfavorable) and _is_unfavorable(zhi_element, unfavorable): matched += 1; factors.append("冲+忌神出行")
            if ten_god in ("偏财", "七杀"): matched += 1; factors.append(f"{ten_god}动象")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 12
        ...
        # ---- 车辆驾驶/安全 ----
            ...
            has_year_hour_clash = has_clash and (zhi == chart.get("pillars", {}).get("year", {}).get("zhi", "") or zhi == chart.get("pillars", {}).get("hour", {}).get("zhi", ""))
            if has_year_hour_clash: matched += 1; factors.append("冲年/时支出行")
            if ten_god == "七杀": matched += 1; factors.append("七杀压力")
            if has_clash: matched += 1; factors.append("地支冲动")
            if _is_unfavorable(gan_element, unfavorable) or _is_unfavorable(zhi_element, unfavorable): matched += 1; factors.append("忌神参与")
            if gan_element in ("火", "金") or zhi_element in ("火", "金"): matched += 1; factors.append("火金工具象")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 12
        ...
        # ---- 合同文书 ----
            ...
            total = 5
            if is_guan and _is_unfavorable(gan_element, unfavorable): matched += 1; factors.append("官杀为忌")
            if ten_god == "伤官": matched += 1; factors.append("伤官见官")
            if has_clash and zhi == chart.get("pillars", {}).get("month", {}).get("zhi", ""): matched += 1; factors.append("冲月支工作")
            if is_guan: matched += 1; factors.append(f"{ten_god}官杀")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 12
        ...
        # ---- 官非口舌/罚单投诉 ----
            ...
            total = 6
            if is_guan: matched += 1; factors.append("官杀规则压力")
            if ten_god == "伤官": matched += 1; factors.append("伤官表达冲规则")
            if ten_god == "七杀" and has_clash: matched += 1; factors.append("七杀遇冲")
            if gan_element == "金" or zhi_element == "金": matched += 1; factors.append("金象规则罚单")
            if _is_unfavorable(gan_element, unfavorable) or _is_unfavorable(zhi_element, unfavorable): matched += 1; factors.append("忌神参与")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 11
        ...
        # ---- 感情推进 ----
            ...
            total = 4
            is_cai_guan = ten_god in ("正财", "偏财", "正官", "七杀")
            if is_cai_guan and _is_favorable(gan_element, favorable): matched += 1; factors.append("财官+喜用")
            if zhi in ("子", "午", "卯", "酉"): matched += 1; factors.append(f"梅花{zhi}")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 12
        ...
        # ---- 感情摩擦 ----
            ...
            total = 5
            if has_clash and zhi == chart.get("pillars", {}).get("day", {}).get("zhi", ""): matched += 1; factors.append("冲日支配偶宫")
            if is_bi_jie and ten_god_counts.get("劫财", 0) > 1: matched += 1; factors.append("劫财过重")
            if ten_god == "伤官" and ten_god_counts.get("伤官", 0) > 1: matched += 1; factors.append("伤官过重")
            if has_clash and _is_unfavorable(zhi_element, unfavorable): matched += 1; factors.append("冲+忌神关系")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 12
        ...
        # ---- 健康波动 ----
            ...
            total = 4
            over_weak = any(_is_unfavorable(e, unfavorable) for e in [gan_element, zhi_element])
            if over_weak: matched += 1; factors.append("流月为忌神")
            if is_guan and _is_unfavorable(gan_element, unfavorable): matched += 1; factors.append(f"{ten_god}压力")
            if has_clash and _is_unfavorable(zhi_element, unfavorable): matched += 1; factors.append("冲+忌神健康")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 12
        ...
        # ---- 病符小疾/体检复查 ----
            ...
            total = 6
            over_weak = any(_is_unfavorable(e, unfavorable) for e in [gan_element, zhi_element])
            if over_weak: matched += 1; factors.append("忌神压身")
            if is_guan: matched += 1; factors.append(f"{ten_god}压力")
            if has_clash: matched += 1; factors.append("冲动身体状态")
            if gan_element in ("火", "土", "金") or zhi_element in ("火", "土", "金"): matched += 1; factors.append("火土金压力")
            if is_yin: matched += 1; factors.append("印星调养信号")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 11
        ...
        # ---- 身体检查/小病早看 ----
            ...
            if over_weak: matched += 1; factors.append("忌神压身")
            if is_guan: matched += 1; factors.append(f"{ten_god}压力")
            if is_yin: matched += 1; factors.append("印星调养")
            if has_clash: matched += 1; factors.append("冲动身体宫位")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 12
        ...
        # ---- 学习考试 ----
            ...
            total = 4
            if is_yin and _is_favorable(gan_element, favorable): matched += 1; factors.append("印星+喜用")
            if _is_favorable(zhi_element, favorable) and is_yin: matched += 1; factors.append("地支喜用")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 12
        ...
        # ---- 合作 ----
            ...
            total = 4
            if is_bi and has_clash: matched += 1; factors.append("比劫+冲")
            if is_bi and _is_favorable(gan_element, favorable): matched += 1; factors.append("比劫+喜用")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 12
        ...
        # ---- 酒局人情/朋友应酬 ----
            ...
            total = 5
            if is_bi_jie: matched += 1; factors.append("比劫朋友人情")
            if zhi in ("子", "午", "卯", "酉"): matched += 1; factors.append("桃花地支人际")
            if has_clash and zhi == chart.get("pillars", {}).get("day", {}).get("zhi", ""): matched += 1; factors.append("冲日支口舌")
            if _is_unfavorable(gan_element, unfavorable) or _is_unfavorable(zhi_element, unfavorable): matched += 1; factors.append("忌神带来消耗")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 12
        ...
        # ---- 人情请托/朋友求助 ----
            ...
            total = 6
            if is_bi_jie: matched += 1; factors.append("比劫朋友同辈")
            if ten_god == "劫财": matched += 1; factors.append("劫财人情资源")
            if has_clash: matched += 1; factors.append("冲动关系位")
            if _is_unfavorable(gan_element, unfavorable) or _is_unfavorable(zhi_element, unfavorable): matched += 1; factors.append("忌神带来消耗")
            if ten_god_counts.get("劫财", 0) + ten_god_counts.get("比肩", 0) >= 2: matched += 1; factors.append("原局比劫不弱")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 11
        ...
        # ---- 家庭事务/长辈事务 ----
            ...
            total = 6
            is_tu = zhi_element == "土" or gan_element == "土"
            if has_clash and zhi == chart.get("pillars", {}).get("year", {}).get("zhi", ""): matched += 1; factors.append("冲年支家庭位")
            if is_yin: matched += 1; factors.append("印星家庭承载")
            if is_tu: matched += 1; factors.append("土象家宅")
            if has_clash: matched += 1; factors.append("冲动旧事")
            if ten_god in ("正印", "偏印", "正财", "偏财"): matched += 1; factors.append(f"{ten_god}关联家庭资产")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 11
        ...
        # ---- 项目财务转机 ----
            ...
            total = 5
            if is_cai: matched += 1; factors.append("财星机会")
            if is_shishen: matched += 1; factors.append("食伤生财")
            if _is_favorable(gan_element, favorable) or _is_favorable(zhi_element, favorable): matched += 1; factors.append("喜用助力")
            if yearly_data and yearly_data.get("relation_to_favorable") == "喜用相关": matched += 1; factors.append("年度喜用加持")
            if evt_type in primary_pool: matched += 1; factors.append(f"十神<{ten_god}>主事件池")
            score = 15 + matched * 12
        ...
        # ---- 其他事件默认评分 ----
            ...
            if evt_type in primary_pool:
                matched = 1; score = 25; factors.append(f"十神<{ten_god}>主事件")
            else:
                continue
    ...
    # ---- Step 3: 地支冲击事件加分 ----
    if has_clash:
        clash_events = _get_clash_driven_events(zhi, chart, ten_god, gan_element, zhi_element, favorable, unfavorable)
    ...
    for evt_type, score in sorted_events:
        ...
        top_events.append(_enrich_event_payload({
            "event_type": evt_type,
            "label": evt_info["label"],
            "category": evt_info["category"],
            "probability_level": probability_level,
            "score": round(score, 1),
            "reason": _generate_reason(evt_type, ten_god, gan_element, zhi_element, has_clash, zhi),
            "advice": _generate_advice(evt_type),
            "trigger_factors": factors,
            "source_ids": EXTENDED_SOURCE_IDS,
        }, ten_god, gan_element, zhi_element, has_clash))
    ...
    return {
        ...
        "gan_element": gan_element,
        "zhi_element": zhi_element,
        "relation_to_favorable": "喜用相关" if any(_is_favorable(e, favorable) for e in [gan_element, zhi_element]) else "忌神相关" if any(_is_unfavorable(e, unfavorable) for e in [gan_element, zhi_element]) else "平稳观察",
