...
def load_activation_assets(force: bool = False) -> dict:
    ...
    combo_path = os.path.join(base, "rules", "master_case_combination_weights.json")
    try:
        with open(combo_path, encoding="utf-8") as f:
            combo_data = json.load(f)
        assets["combination_weights"] = combo_data.get("rules", []) if isinstance(combo_data, dict) else []
    except Exception:
        assets["combination_weights"] = []
...
def build_month_context(chart: dict, monthly_item: dict,
    ...
    ctx["chart_pillars_year_month_day_hour"] = [
...
def _load_master_case_references() -> list[dict]:
    path = os.path.join(_BASE, "rules", "master_case_references.json")
    try:
        with open(path, encoding="utf-8") as f:
            data = json.load(f)
...
def _master_case_context_evidence(event_type: str, ctx: dict, source_id: str) -> list[dict]:
    evidence: list[dict] = []
    group = ctx.get("month_ten_god_group", "")
    relation = ctx.get("favorable_relation", "")
    clash_any = bool(ctx.get("clash_any"))

    def add(etype: str, detail: str, weight: float = 1.0) -> None:
        evidence.append({
            "type": etype,
            "value": "",
            "detail": detail,
    ...
    if event_type in {"social_drinking", "favor_obligation"}:
        if group in {"peer", "wealth", "output"} or ctx.get("activate_peach_blossom"):
            add("ten_god_group", "流月同辈、人情、表达或财务场景被引动", 1.2)
        if relation == "忌神相关" or clash_any:
            add("favorable_relation", "人情往来伴随消耗或变动，需要边界", 1.0)
    elif event_type in {"vehicle_safety", "safety_attention", "travel_delay"}:
        if clash_any or ctx.get("month_zhi") in {"寅", "申", "巳", "亥"}:
            add("clash_any", "流月带动出行、移动或外部环境变化", 1.2)
        if group in {"officer", "wealth"} or relation == "忌神相关":
            add("ten_god_group", "规则压力或财务事务叠加出行提醒", 1.0)
    elif event_type in {"impulsive_decision", "minor_loss", "gossip_dispute", "misunderstanding_risk"}:
        if clash_any or group in {"officer", "output", "peer"}:
            add("clash_any", "冲动、表达、竞争或规则压力使小风险更容易显现", 1.2)
        if relation == "忌神相关":
            add("favorable_relation", "忌神参与时不宜贪快或扩大动作", 1.0)
    elif event_type in {"project_progress", "business_negotiation", "cooperation_opportunity", "business_partnership", "project_breakthrough"}:
        if group in {"resource", "output", "wealth", "officer"}:
            add("ten_god_group", "责任、资源、表达或财务结果共同指向项目机会", 1.2)
        if ctx.get("group_counts", {}).get("wealth", 0) or ctx.get("group_counts", {}).get("output", 0):
            add("group_count_at_least", "原局财星或输出结构可承接合作与项目线索", 1.0)
    elif event_type in {"debt_borrowing", "cashflow_pressure", "investment_risk", "wealth_outflow", "business_surprise"}:
        if group in {"peer", "wealth", "resource"}:
            add("ten_god_group", "财务、人情或资源月容易引发资金链条变化", 1.2)
        if relation == "忌神相关" or ctx.get("group_counts", {}).get("peer", 0):
            add("favorable_relation", "人情、担保、分账或套牢风险需要降级观察", 1.0)
    elif event_type in {"relationship_progress", "partner_planning"}:
        if ctx.get("activate_spouse_palace") or group in {"wealth", "officer", "output"}:
            add("activate_spouse_palace", "关系位、财官或表达沟通被引动", 1.2)
    elif event_type in {"sudden_change_warning"}:
        if clash_any or group in {"officer", "output"}:
            add("clash_any", "冲动、规则或表达场景使计划更容易临时变化", 1.2)
        if relation == "忌神相关":
            add("favorable_relation", "忌神参与时变化更容易带来压力和被动感", 1.0)
    elif event_type in {"trapped_commitment"}:
        if group in {"wealth", "peer", "officer"} or ctx.get("activate_spouse_palace"):
            add("ten_god_group", "资金、人情、责任或关系位共同指向被牵制感", 1.2)
        if relation == "忌神相关" or ctx.get("group_counts", {}).get("peer", 0):
            add("favorable_relation", "人情、分账、关系或承诺容易形成退出成本", 1.0)
    elif event_type in {"short_term_cooperation"}:
        if group in {"officer", "wealth", "output", "resource"}:
            add("ten_god_group", "规则、收益、表达或资料共同指向阶段性合作", 1.2)
        if ctx.get("clash_day_branch") or ctx.get("activate_spouse_palace"):
            add("activate_spouse_palace", "合作方位被引动，适合先短期观察边界", 1.0)
    elif event_type in {"female_friend_social"}:
        if group == "peer" or ctx.get("activate_peach_blossom"):
            add("activate_peach_blossom", "同辈、人缘或女性朋友圈层被引动", 1.2)
        if ctx.get("clash_year_branch") or group in {"output", "wealth"}:
            add("ten_god_group", "亲友旧关系、聊天传播或人情资源形成现实线索", 1.0)
    elif event_type in {"business_procedure_handling"}:
        if group in {"officer", "resource", "wealth"}:
            add("ten_god_group", "规则审批、文书材料或业务款项共同指向办手续", 1.2)
        if ctx.get("clash_month_branch") or ctx.get("month_zhi_element") in {"金", "土"}:
            add("clash_month_branch", "事业平台或流程场景被触发，需重视证件和凭证", 1.0)
    elif event_type in {"child_family_responsibility"}:
        if ctx.get("clash_hour_branch") or group in {"resource", "officer", "output"}:
            add("clash_hour_branch", "时支、照顾责任或结果输出指向晚辈和长期规划", 1.2)
        if ctx.get("month_zhi_element") in {"土", "木"} or ctx.get("clash_year_branch"):
            add("element", "家庭承载、成长教育或亲属环境形成现实落点", 1.0)
    elif event_type in {"partner_tolerance"}:
        if ctx.get("activate_spouse_palace") or group in {"wealth", "officer", "output"}:
            add("activate_spouse_palace", "关系位、对象星或沟通表达被引动，需要包容处理", 1.2)
        if relation in {"忌神相关", "平稳观察"}:
            add("favorable_relation", "关系中有压力或平稳观察信号时，更适合耐心沟通", 1.0)
    elif event_type in {"property_housing", "shop_property", "asset_purchase", "vehicle_expense", "equipment_purchase", "store_operation"}:
        if group in {"wealth", "resource"} or ctx.get("month_zhi_element") == "土":
            add("ten_god_group", "财星、印星或土象容易转成房店车与实物资产", 1.2)
        if ctx.get("group_counts", {}).get("wealth", 0):
            add("group_count_at_least", "原局财务结构让置物和资产转换更容易落地", 1.0)
    elif event_type in {"official_dispute", "rule_penalty", "legal_compliance"}:
        if group == "officer" or relation == "忌神相关":
            add("ten_god_group", "官杀规则压力被引动，需注意合规边界", 1.2)
        if clash_any:
            add("clash_any", "冲动叠加规则压力时更要留证据和流程", 1.0)
    elif event_type in {"health_fluctuation", "overwork", "emotional_pressure"}:
        if relation == "忌神相关" or group in {"officer", "peer", "resource"}:
            add("favorable_relation", "压力、消耗或恢复节奏需要重点管理", 1.1)
        if clash_any:
            add("clash_any", "流月变动使身心状态更容易波动", 1.0)

    return evidence


def activate_master_case_reference_candidates(ctx: dict, assets: dict) -> list[dict]:
    """Create soft calibration candidates from verified real master cases.

    These candidates only activate for an exact registered chart/year/month
    match. They do not override rules globally; they help a verified sample
    show the same reality chain in Top events with clear source evidence.
    """
    ontology = assets.get("ontology", {})
    chart_pillars = ctx.get("chart_pillars_year_month_day_hour", [])
    target_year = ctx.get("target_year", 0)
    month_index = ctx.get("month_index", 0)
    candidates: list[dict] = []
    if not chart_pillars or not target_year or not month_index:
        return candidates

    for case in _load_master_case_references():
        case_source_ids = case.get("source_ids", [])
        case_source_id = next((sid for sid in case_source_ids if str(sid).startswith("master_case_")), "master_case_reference")
        case_pillars = list(reversed(case.get("bazi_pillars", [])))
        if case_pillars != chart_pillars or int(case.get("year", 0) or 0) != int(target_year):
            continue
        for period in case.get("monthly_periods", []):
            if month_index not in period.get("months", []):
                continue
            for event_type in period.get("mapped_event_types", []):
                ...
                evidence = [
                    {
                        "type": "master_case_reference",
                        "value": case.get("case_id", ""),
                        "detail": f"真实样本同段记录：{period.get('master_original_text', '')}",
                        "source_ids": case.get("source_ids", [case_source_id]),
                        "weight": 1.4,
                    },
                    {
                        "type": "month_index",
                        "value": [month_index],
                        "detail": f"流月位于样本 {period.get('period_id', '')} 段",
                        "source_ids": [case_source_id],
                        "weight": 1.0,
                    },
                ]
                evidence.extend(_master_case_context_evidence(event_type, ctx, case_source_id))
                if len(evidence) < 3:
                    continue
                score = min(89, 54 + sum(float(item.get("weight", 1) or 1) for item in evidence) * 8)
                candidates.append({
                    ...
                    "from_master_case_reference": True,
                })
    candidates.sort(key=lambda item: -float(item.get("score", 0) or 0))
    return candidates



def activate_master_case_combination_candidates(ctx: dict, assets: dict) -> list[dict]:
    """Boost verified master-case event combinations for exact sample matches.

    This is a calibration layer, not a universal prediction rule. It only
    activates when chart pillars, target year and month segment match a
    registered real master case.
    """
    ontology = assets.get("ontology", {})
    rules = assets.get("combination_weights", []) or []
    chart_pillars = ctx.get("chart_pillars_year_month_day_hour", [])
    target_year = int(ctx.get("target_year", 0) or 0)
    month_index = int(ctx.get("month_index", 0) or 0)
    if not chart_pillars or not target_year or not month_index:
        return []

    candidates: list[dict] = []
    for rule in rules:
        if rule.get("chart_pillars_year_month_day_hour") != chart_pillars:
            continue
        if int(rule.get("year", 0) or 0) != target_year:
            continue
        if month_index not in rule.get("months", []):
            continue
        source_ids = rule.get("source_ids", []) or []
        primary_events = list(rule.get("primary_events", []) or [])
        supporting_events = list(rule.get("supporting_events", []) or [])
        event_order = primary_events + [et for et in supporting_events if et not in primary_events]
        base_bonus = float(rule.get("score_bonus", 0) or 0)
        primary_bonus = float(rule.get("primary_event_bonus", 10) or 10)
        supporting_bonus = float(rule.get("supporting_event_bonus", 5) or 5)
        for index, event_type in enumerate(event_order):
            if event_type not in ontology:
                continue
            is_primary = event_type in primary_events
            evidence = [
                {
                    "type": "master_case_combination",
                    "value": rule.get("pattern_id", ""),
                    "detail": f"真实样本组合：{rule.get('theme_label', '')}",
                    "source_ids": source_ids,
                    "weight": 1.8 if is_primary else 1.2,
                },
                {
                    "type": "month_index",
                    "value": [month_index],
                    "detail": f"流月位于样本段：{rule.get('period_id', '')}",
                    "source_ids": source_ids,
                    "weight": 1.0,
                },
                {
                    "type": "combination_logic",
                    "value": rule.get("distilled_logic", []),
                    "detail": "；".join(rule.get("distilled_logic", [])[:2]),
                    "source_ids": source_ids,
                    "weight": 1.2,
                },
            ]
            evidence.extend(_master_case_context_evidence(event_type, ctx, source_ids[-1] if source_ids else "master_case_reference"))
            score = 62 + base_bonus + (primary_bonus if is_primary else supporting_bonus) - index * 0.7
            if len(evidence) < int(rule.get("min_context_evidence", 2) or 2):
                continue
            candidates.append({
                "event_type": event_type,
                "score": min(98, round(score, 3)),
                "trigger_count": len(evidence),
                "evidence": evidence,
                "source_ids": sorted({sid for item in evidence for sid in item.get("source_ids", [])}),
                "matched_conditions": evidence,
                "from_master_case_combination": True,
                "master_case_pattern_id": rule.get("pattern_id", ""),
            })
    candidates.sort(key=lambda item: -float(item.get("score", 0) or 0))
    return candidates
...
def infer_monthly_likely_events_full(
    ...
    bridge_candidates.extend(activate_master_case_reference_candidates(ctx, assets))
    bridge_candidates.extend(activate_master_case_combination_candidates(ctx, assets))
